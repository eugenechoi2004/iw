...
def plot_state_pair_embeddings(encoder, env, manifold, device, epoch=0, save_dir="plots"):
    """
    Plot embeddings of specific state pairs on the Poincaré Ball, labeling each point with the node pair (s, g).

    Parameters:
    - encoder: The neural network encoder for state embeddings.
    - env: The N-ary tree environment to sample states from.
    - manifold: The hyperbolic manifold (e.g., PoincareBall).
    - device: The device (CPU/GPU) for tensor operations.
    - epoch: Current epoch number (for naming the saved file).
    - save_dir: Directory where plots will be saved (default: 'plots').
    """
    ...
    # Compute embeddings for all same-state pairs phi(s, s)
    with torch.no_grad():
        phi_ss = encoder(states, states)  # phi(s, s)

    # Convert to Euclidean coordinates if needed
    if hasattr(phi_ss, 'tensor'):
        phi_ss = phi_ss.tensor

    phi_ss_np = phi_ss.cpu().numpy()

    # Plot phi(s, s) embeddings
    ax.scatter(phi_ss_np[:, 0], phi_ss_np[:, 1], color='blue', label=r'$\phi(s, s)$', alpha=0.7)

    # Now select specific state pairs phi(s, g) where s != g
    # For clarity, we'll select a few specific node pairs and plot them
    specific_pairs = [
        (3, 5),
        (7, 14),
        (9, 11),
    ...
    with torch.no_grad():
        phi_sg = encoder(s_tensors, g_tensors)  # phi(s, g)
    ...
    # Plot phi(s, g) embeddings and label them with node pairs
...
for epoch in range(num_epochs):
    ...
    for batch in data_loader:
        ...
        # Encode start-goal
        sg_encoded = encoder(start.long(), goal.long())  # [batch_size, embedding_dim]

        # Encode waypoint-waypoint
        ww_encoded = encoder(waypoint.long(), waypoint.long())  # [batch_size, embedding_dim]
        ...
        # Negate distances to get similarities
